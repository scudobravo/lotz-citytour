...
const startTour = () => {
    ...
    const mapUrl = pointsOfInterest.value.length > 0
        ? `https://www.google.com/maps/directions/${pointsOfInterest.value[0].latitude},${pointsOfInterest.value[0].longitude}/${pointsOfInterest.value.slice(1).map(p => `${p.latitude},${p.longitude}`).join('/')}?travelmode=walking`
